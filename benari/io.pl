...
%  numbervars(Fml1, x, 1, _),     % Instantiate variables for output.
  numbervars(Fml1, 1, _,[functor_name(x)]), % JB
  write_formula1(Fml1).
...
%  numbervars(List1, x, 1, _),     % Instantiate variables for output.
  numbervars(List1, 1, _,[functor_name(x)]), % JB
  write('['),
...
%  numbervars((L1, L2), x, 1, _),
   numbervars((L1, L2), 1, _,[functor_name(x)]), % JB
...
write_latex(Fml) :-
  copy_term(Fml, Fml1),          % Do not instantiate original formula.
%  numbervars(Fml1, x, 1, _),     % Instantiate variables for output.
%  numbervars(Fml1, 0, _),     % JB - Instantiate variables for output.
%  numbervars(Fml1, 1, _,[singletons(true)]), % JB
  numbervars(Fml1, 1, _,[functor_name(x)]), % JB
  write_latex1(Fml1).
